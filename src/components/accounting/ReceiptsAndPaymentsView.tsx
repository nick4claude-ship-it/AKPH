    ...
    <div className="space-y-4 animate-in fade-in duration-150">
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 font-bold px-3.5 py-2 rounded-lg text-xs transition-colors cursor-pointer shadow-2xs text-white ${
            type === 'receipts' ? 'bg-teal-600 hover:bg-teal-700' : 'bg-rose-600 hover:bg-rose-700'
          }`}
        >
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          {type === 'receipts' ? (
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold">
                <tr>
                  ...
                  <th className="py-3 px-3 font-mono">شماره پیگیری</th>
                  <th className="py-3 px-3 font-mono">سند حسابداری</th>
                  <th className="py-3 px-3 font-mono text-left text-teal-800">مبلغ ({unit})</th>
                </tr>
              </thead>
                ...
                {filteredReceipts.map((rec) => (
                  <tr key={rec.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-3 font-sans">
                      <span className="text-[10px] bg-slate-100 text-slate-700 px-2 py-0.5 rounded font-semibold">{rec.method}</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{rec.trackingNumber}</td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{rec.journalEntryId || '-'}</td>
            ...
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold">
                <tr>
                  <th className="py-3 px-4 font-mono">سند حسابداری</th>
                  <th className="py-3 px-3">تاریخ</th>
                  <th className="py-3 px-4">ذی‌نفع</th>
                  <th className="py-3 px-3">پروژه</th>
                  <th className="py-3 px-3">پرداخت از</th>
                  <th className="py-3 px-3 font-mono">مرجع</th>
                  <th className="py-3 px-3 font-mono">شماره پیگیری</th>
                  <th className="py-3 px-3 font-mono text-left text-rose-800">مبلغ ({unit})</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-mono">
                {filteredPayments.map((pay) => (
                  <tr key={pay.id} className="hover:bg-slate-50">
                    <td className="py-3 px-4 font-bold text-slate-900">{pay.journal}</td>
                    ...
                    <td className="py-3 px-3 font-sans text-slate-600">{pay.project}</td>
                    <td className="py-3 px-3 font-sans text-slate-600 text-[11px]">{pay.account}</td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{pay.reference}</td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{pay.tracking}</td>
        ...
        {(type === 'receipts' ? filteredReceipts : filteredPayments).length === 0 && (
          <p className="py-10 text-center text-xs text-slate-400">موردی یافت نشد.</p>
        )}
      </div>
    </div>
  );
