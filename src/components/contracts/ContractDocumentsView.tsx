    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div>
          <h2 className="text-base font-bold text-slate-900">
            مخزن اسناد، مدارک فنی و پیوست‌های قراردادها
          </h2>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200 text-sm">
        <div className="relative w-full sm:w-80">
          ...
          <input aria-label="جستجو در عنوان فایل، نوع مدرک یا ثبت‌کننده"
            type="text"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-bold text-slate-900 block truncate" title={doc.fileName}>
                    {formatText(doc.fileName)}
                  </span>
                  <span className="text-xs text-slate-500 block mt-1">
                    {formatText(contract?.code)} · {contract?.projectTitle.slice(0, 25)}...
                  </span>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="px-2 py-1 rounded text-xs font-bold bg-slate-100 text-slate-700">
                      {formatText(doc.fileType)}
                    </span>
                    <span className="text-xs text-slate-500 tabular-nums">نسخه {formatText(doc.version)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span>
                  {formatText(doc.uploadDate)} · {formatText(doc.fileSize)}
                </span>
                <button
                  disabled
                  className="p-2 rounded-lg text-slate-500 cursor-not-allowed flex items-center gap-1 font-bold text-xs"
