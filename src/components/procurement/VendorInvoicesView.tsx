          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                تطبیق مکانیزه ۳ سند کلیدی: سفارش خرید + قبض انبار و باسکول + فاکتور رسمی فروشنده
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pr-9 pl-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-hidden"
            />
          </div>

          <div>
            <select aria-label="فیلتر: تمام وضعیت‌های تطبیق و پرداخت"
              value={selectedStatus}
              onChange={(e) => setSelectedStatus(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-hidden"
            >
              ...
              {filteredInvoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-3 px-4">
                    <div className="tabular-nums font-bold text-slate-900">{formatText(inv.invoiceNumber)}</div>
                    <div className="text-xs text-slate-500 mt-1">
                      سررسید: <span className="tabular-nums font-bold text-slate-700">{formatText(inv.dueDate)}</span>
                    </div>
                  </td>

                  <td className="py-3 px-4">
                    <div className="font-bold text-slate-800">{formatText(inv.supplierName)}</div>
                    <div className="text-xs text-slate-500 mt-1">{formatText(inv.projectName)}</div>
                  </td>

                  <td className="py-3 px-4 text-slate-600">
                    <div className="flex items-center gap-1 tabular-nums text-sm">
                      <span className="text-slate-500">PO:</span>
                      <span className="font-bold text-indigo-700">{formatText(inv.poNumber)}</span>
                    </div>
                    {inv.grnNumber ? (
                      <div className="flex items-center gap-1 tabular-nums text-sm text-emerald-700">
                        <span className="text-slate-500">GRN:</span>
                        <span className="font-bold">{formatText(inv.grnNumber)}</span>
                      </div>
                    ) : (
                      <span className="text-sm text-rose-700 font-bold">بدون رسید انبار</span>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="inline-flex flex-col items-center gap-1">
                      ...
                      >
                        {formatText(inv.threeWayMatching.status)}
                      </span>
                      {inv.threeWayMatching.qtyVarianceAmount > 0 && (
                        <span className="text-sm text-rose-700 font-bold">
                          کسر <Money rial={inv.threeWayMatching.qtyVarianceAmount} /> مغایرت
                        </span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-4 text-left tabular-nums font-bold text-slate-900">
                    <Money rial={inv.totalAmount} />
                  </td>

                  <td className="py-3 px-4 text-left tabular-nums font-bold">
                    <span className={inv.remainingBalance > 0 ? 'text-rose-700' : 'text-emerald-700'}>
                      <Money rial={inv.remainingBalance} />
                    </span>
                  </td>

                  <td className="py-3 px-4 text-center">
                    {inv.accountingEntryNumber ? (
                      <span className="tabular-nums text-xs px-2 py-1 bg-slate-100 text-slate-700 rounded font-bold">
                        {formatText(inv.accountingEntryNumber)}
                      </span>
                    ) : (
                      <span className="text-sm text-amber-700 font-bold">در صف صدور سند</span>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-4 text-center">
                    <button
                      ...
                      className="px-2 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                    >
                      بررسی تطبیق
                    </button>
                  </td>
        ...
        <Dialog onClose={() => setActiveInvoiceForDetail(null)} label="پانل تطبیق سه‌جانبه فاکتور" overlayClassName="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4" className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-200 bg-slate-50 rounded-t-2xl">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-800">
                    پانل تطبیق سه‌جانبه فاکتور {formatText(activeInvoiceForDetail.invoiceNumber)}
                  </h3>
                  <p className="text-xs text-slate-500">{formatText(activeInvoiceForDetail.supplierName)} - {formatText(activeInvoiceForDetail.projectName)}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2">
                  <div className="flex items-center justify-between text-slate-700 font-bold border-b border-slate-200 pb-2">
                    <span>۱. سفارش خرید</span>
                    <span className="tabular-nums text-sm text-indigo-700">{formatText(activeInvoiceForDetail.poNumber)}</span>
                  </div>
                  <div className="text-sm text-slate-600">قیمت و شرایط تأیید شده توسط دفتر مرکزی</div>
                  <div className="flex items-center gap-1 text-emerald-700 font-bold text-sm pt-1">
                ...
                <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2">
                  <div className="flex items-center justify-between text-slate-700 font-bold border-b border-slate-200 pb-2">
                    <span>۲. رسید انبار</span>
                    <span className="tabular-nums text-sm text-emerald-700">
                      {formatText(activeInvoiceForDetail.grnNumber || 'فاقد قبض')}
                    </span>
                  </div>
                  <div className="text-sm text-slate-600">قبض باسکول و تست آزمایشگاه QC کارگاه</div>
                  <div className="flex items-center gap-1 font-bold text-sm pt-1">
                ...
                <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2">
                  <div className="flex items-center justify-between text-slate-700 font-bold border-b border-slate-200 pb-2">
                    ...
                    <span className="tabular-nums text-sm text-slate-900">{formatText(activeInvoiceForDetail.invoiceNumber)}</span>
                  </div>
                  <div className="text-sm text-slate-600">شناسه مودیان: {formatText(activeInvoiceForDetail.taxRegistrationNumber)}</div>
                  <div className="flex items-center gap-1 text-indigo-700 font-bold text-sm pt-1">
              ...
              <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/60 space-y-2">
                <div className="flex justify-between items-center text-slate-600">
                  ...
                  <span className="tabular-nums font-bold text-slate-900"><Money rial={activeInvoiceForDetail.subtotal} /></span>
                </div>
                <div className="flex justify-between items-center text-slate-600">
                  ...
                  <span className="tabular-nums font-bold text-slate-900"><Money rial={activeInvoiceForDetail.vatAmount} /></span>
                </div>
                <div className="flex justify-between items-center text-slate-600">
                  <span>هزینه حمل و تخلیه:</span>
                  <span className="tabular-nums font-bold text-slate-900"><Money rial={activeInvoiceForDetail.shippingCost} /></span>
                </div>
                <div className="border-t border-slate-200 pt-2 flex justify-between items-center font-bold text-slate-900 text-sm">
                  ...
                  <span className="tabular-nums text-indigo-700 text-base">
                    <Money rial={activeInvoiceForDetail.totalAmount} />
                  </span>
                </div>
                <div className="flex justify-between items-center text-emerald-700 font-bold pt-1">
                  ...
                  <span className="tabular-nums"><Money rial={activeInvoiceForDetail.paidAmount} /></span>
                </div>
                <div className="border-t-2 border-slate-300 pt-2 flex justify-between items-center font-bold text-rose-700 text-sm">
                  <span>مانده قابل تسویه / صدور چک صیادی:</span>
                  <span className="tabular-nums"><Money rial={activeInvoiceForDetail.remainingBalance} /></span>
                </div>
              </div>

              {/* Notes & Variance Analysis */}
              <div className="border border-slate-200 rounded-xl p-3 bg-white space-y-2">
                <span className="font-bold text-slate-800 block text-sm">گزارش سیستم تطبیق و مغایرت:</span>
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl flex justify-between items-center">
              ...
              <button
                onClick={() => setActiveInvoiceForDetail(null)}
                className="px-4 py-2 border border-slate-300 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-200/50 transition-colors cursor-pointer mr-auto"
              >
                بستن
              </button>
            </div>
