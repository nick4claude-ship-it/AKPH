...
import { formatCurrencyCompact, formatDecimal, formatText } from '../utils/formatters';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        <button
          onClick={() => setKind('all')}
          className={`px-3 py-2 rounded-lg border cursor-pointer ${kind === 'all' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white border-slate-200 text-slate-600'}`}
        >
        ...
        {counts.map(([k, n]) => (
          <button
            key={k}
            onClick={() => setKind(k)}
            className={`px-3 py-2 rounded-lg border cursor-pointer ${kind === k ? 'bg-slate-900 text-white border-slate-900' : 'bg-white border-slate-200 text-slate-600'}`}
          >
      ...
      <div className="space-y-2.5">
        ...
        {visible.map((n) => {
          ...
          return (
            <div key={n.id} className={`rounded-xl border p-3 flex items-start justify-between gap-3 ${style.box} ${isDismissed ? 'opacity-60' : ''}`}>
              <div className="flex items-start gap-3 min-w-0">
                <Icon className="w-4 h-4 mt-1 shrink-0 text-slate-600" />
                <div className="min-w-0 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`text-xs font-bold px-2 py-1 rounded ${style.badge}`}>{formatText(style.label)}</span>
                    <span className="text-xs text-slate-500 bg-white/70 px-2 py-1 rounded border border-slate-200">{KIND_LABELS[n.kind]}</span>
                    <h3 className="text-sm font-bold text-slate-900">{formatText(n.title)}</h3>
                  </div>
                  <p className="text-sm text-slate-600 leading-relaxed">{formatText(n.description)}</p>
                  <div className="text-xs text-slate-500 flex gap-3">
                    {n.relatedProjectName && <span>پروژه: {formatText(n.relatedProjectName)}</span>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                {n.actionPath && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold px-2 py-2 rounded-lg bg-slate-900 text-white hover:bg-slate-800 cursor-pointer"
                  >
                    <ArrowUpLeft className="w-3 h-3" />
                    {formatText(n.actionLabel || 'اقدام')}
                  </button>
                )}
                <button
